...
class HandTracker():
    ...
    def _get_triangle(self, kp0, kp2, dist=1):
        ...
        dir_v_r = np.dot(dir_v, self.R90.T)
    ...
    def __call__(self, img):
        ...
        kp_orig = np.dot(self._pad1(joints), Minv.T)[:,:2]
        box_orig = np.dot(self._target_box, Minv.T)[:,:2]
        kp_orig -= pad[::-1]
        box_orig -= pad[::-1]
